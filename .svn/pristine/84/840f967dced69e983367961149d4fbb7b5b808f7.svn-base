...
const actions = {
  ...
  getrechargeWayList({commit}, data){ // 充值方式列表
    let sendData = {
  ...
  getintegralTList ({commit},data) {  //积分倍率列表
    let userInfo = getUserInfo()
    let sendData = {
  ...
  payMoneyMorePayWay({commit}, data){  //消费结账保存(组合支付)
    let homeInfo = getHomeData()
    let sendData = {
      InterfaceCode : '91020301_1',
      CompanyId: homeInfo.shop.COMPANYID,
  ...
  getDiscountTypeList({commit}, data){  // 折扣类型列表
    let homeInfo = getHomeData()
    let sendData = {
  ...
  saveDiscountType({commit}, data){   // 折扣类型保存
    let homeInfo = getHomeData()
    let sendData = {
  ...
  delDiscountType({commit}, data){   // 折扣类型删除
    let homeInfo = getHomeData()
    let sendData = {
