            ...
            <div class="content-table-center">
              ...
              >
                ...
                <el-table-column label="操作" width="180px">
                  <template slot-scope="scope">
                    <el-button size="small" type="text" @click="handlePassW(scope.row)">
                      密码
                    </el-button>
                    <el-button
                    ...
                    <el-button
                      ...
                      type="text"
                      :disabled="scope.row.ISADMIN"
                      @click="handleDeal('edit', scope.row)"
                    >
                      详情
                    </el-button>
                  </template>
            ...
            >
              ...
              <el-form-item label="店铺权限">
                <div class="full-width">
                  <div
                    class="inline-block m-right-sm m-bottom-sm"
              ...
              <el-form-item label="用户权限">
                <el-button type="text" @click="openPermission">
                  用户权限 {{ userPermissionNum }} 项
                </el-button>
              </el-form-item>
        ...
        <el-dialog title="设置用户权限" :visible.sync="isShowPermission" width="70%">
          <div v-for="(item, key, i) in pageList" :key="i" class="row-flex">
            ...
            <div class="full-width">
              ...
              >
                <el-checkbox
                  v-model="subItem.IsPurview"
                  :label="subItem.MODULENAME"
                  border
                ></el-checkbox>
              </div>
            </div>
...
export default {
  ...
  watch: {
    ...
    dealState(data) {
      this.loading = false;
      if (data.success) {
        let param = this.checkedCities,
          param1 = this.pageList2,
          sendArr1 = [],
          sendArr2 = [];
        ...
        if (data.type == "add" || data.type == "edit") {
          this.$store
            .dispatch("setPermissionStore", {
              uid: this.uid == "" ? data.data.id : this.uid,
              sarr: sendArr2
            })
            .then(() => {
              ...
              this.$store.dispatch("setPermissionList", {
                uid: this.uid == "" ? data.data.id : this.uid,
                parr: this.userPermission,
                BrandList: JSON.stringify(sendArr1)
              });
            });
        }
      ...
      this.$prompt("", "重置密码", {
        ...
        .then(({ value }) => {
          this.$store.dispatch("resetUserPassWord", {
            UserID: row.USERID,
            Password: CRYPTO.MD5(value).toString()
          });
        })
        .catch(() => {});
    },
