      ...
      <el-container>
          ...
          <el-container>
            <div class="content-new-fex"> 
              ...
              <historyPage v-on:isAddNewBill_W='isAddNewBill_W' v-if="current==1"></historyPage>
            </div>
          </el-container>
      </el-container>
...
import addNewPage from "./warehousing/addNew.vue";
import historyPage from "./warehousing/history.vue";
  ...
  methods:{
    ...
    clearSetItem(){
      this.$store.dispatch('cgdWarehousingDataList', '').then(()=>{
        sessionStorage.setItem('theGoodsList', JSON.stringify([]))
        sessionStorage.setItem('theGoodsObj', JSON.stringify({SUPPLIERID:''}))
      })
    }
  },
  destroyed: function () {
