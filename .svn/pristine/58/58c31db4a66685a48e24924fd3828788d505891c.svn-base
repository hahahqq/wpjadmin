<template>
  <div>
    <el-form ref="ruleForm" :model="ruleForm" :rules="rules" inline label-width="100px">
      ...
      <el-form-item label="销售员" class="no-m-right" style="width:49%">
         <el-select placeholder="请选择销售员" v-model="ruleForm.empval" class="full-width" style="width:200px">
            <el-option v-for="(item,i) in employeeList" :key="i" :label="item.NAME" :value="item.ID" size="small"></el-option>
         </el-select>
      </el-form-item>
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      empdataListState: 'empdataListState'
    })
  },
  watch: {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
         ...
         console.log(this.ruleForm)
          ...
          this.$store.dispatch("memberRecharge", this.ruleForm).then(() => {
            this.loading = true;
          });
        } else {
