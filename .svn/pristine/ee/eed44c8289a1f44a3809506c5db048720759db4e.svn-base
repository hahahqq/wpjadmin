...
const actions = {
  ...
  setMarketingList({commit,state}, obj) {
    ...
    state.marketingListState = Object.assign({ 'message':"操作成功", 'success':true},{'paying':pageData});
  },
  ...
  dealPromotionAction({commit,state},{type,data}) { //发布优惠活动, 新增和编辑
    let homeInfo = getHomeData();
    let sendData={
      'InterfaceCode':'',
      'CompanyId':homeInfo.shop.COMPANYID,
    ...
    switch(type){
      case 'add': sendData.InterfaceCode="91020701"; break;
      case 'edit': sendData.InterfaceCode='91020705';sendData.BillId = data.BillId; break;
      // case 'del': break;
    }
    commonSend.commonSend('get', data => {
  ...
  dealGoodsAction({commit,state},{type,data}) { //发布优惠商品, 新增和编辑
    ...
    switch(type){
      case 'add': sendData.InterfaceCode="91020711";   break;
      case 'edit': sendData.InterfaceCode='91020715';sendData.BillId = data.BillId; break;
      // case 'del': break;
    }
    commonSend.commonSend('get', data => {
      commit( DEAL_MARKETING_ACTION, { data })
  ...
  getMarketingListImg({commit}, data) { // 背景列表
    let homeInfo = getHomeData();
    let sendData = {
  ...
  getMarketingSaveImg({commit,state}, data) { // 背景保存(仅允许修改主图以及缩略图)
    let homeInfo = getHomeData();
    let sendData = {
  ...
  getMarketingRechargeList({commit,state}, data) { //充值赠送有礼列表
    let homeInfo = getHomeData();
  ...
  getMarketingRechargeDetailed({commit,state}, data) { //充值赠送有礼详细
    let userInfo = getUserInfo();
    let pn = parseInt(state.marketingRechargeListState.paying.PN) +1;
    let sendData = {
  ...
  getMarketingRechargeStatus({commit}, data) { // 充值有礼状态
    let userInfo = getUserInfo();
    let homeInfo = getHomeData()
    let sendData = {
  ...
  getMarketingRechargeAdd({commit}, data) {  // 发布充值有礼
    let userInfo = getUserInfo();
    let sendData = {
  ...
  getMarketingRechargeEit({commit}, data) { // 活动充值修改
    let userInfo = getUserInfo();
    let sendData = {
  ...
  getMarketingRechargeDel({commit}, data) { // 活动充值删除
    let userInfo = getUserInfo();
    let sendData = {
