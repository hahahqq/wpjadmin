...
const actions = {
    ...
    getsizeGroupList({commit}, data){  // 尺码组列表
        let homeInfo = getHomeData()
        let sendData = {
    ...
    addSizeGroup({commit}, data){  // 尺码组保存\修改
        let homeInfo = getHomeData()
        let sendData = {
    ...
    delSizeGroupList({commit}, data){ // 尺码组删除
        let homeInfo = getHomeData()
        let sendData = {
    ...
    getsizeaddState({commit}, data) {  // 保存尺寸
        let homeInfo = getHomeData();
        let sendData={
    ...
    getsizeEditState({commit}, data) {  // 修改尺寸
        let homeInfo = getHomeData();
        let sendData={
            'InterfaceCode': '9102080203',
            'CompanyId': homeInfo.shop.COMPANYID,
    ...
    getDelsizeState({commit}, data){ //删除尺寸
        let homeInfo = getHomeData()
        let sendData = {
    ...
    sizeCodeGroupSave({ commit }, data){  //尺码编号批量保存
        let homeInfo = getHomeData();
        let sendData={
      ...
      getSizeBarCodeList({ commit }, data){  //尺码编号列表
        let homeInfo = getHomeData();
        let sendData={
...
const mutations = {
    ...
    [DEL_SIZE_GROUP](state, {data}){  // 删除尺码组
        state.delSizeGroupState = Object.assign({}, data)
    },
    [ADD_SIZE_GROUP](state,{data}){  // 保存尺码组
