  ...
  <div class="report row-flex flex-start m-top-sm">
    <section class="bg-white marginLR-sm paddingTB-sm paddingLR-md full-width" v-loading="loading">
      ...
      <div class='full-width pull-left m-bottom-sm reportInfo'>
        共盘点 <span>{{dataObj.NUM ? dataObj.NUM : 0 }}</span> 笔 , 
        商品数  <span>{{dataObj.GOODSQTY ? dataObj.GOODSQTY : 0}}</span>
        盘点数量 <span>{{dataObj.QTY ? dataObj.QTY : 0}}</span> ,
        盈亏数量 <span>{{dataObj.STOCKQTY ? dataObj.STOCKQTY : 0}}</span>
        盈亏金额 <span>{{dataObj.MONEY ? dataObj.MONEY : 0}}</span>,
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      dataObj:{ NUM : 0, QTY : 0, MONEY : 0, GOODSQTY: 0, STOCKQTY : 0 },
      exportLoading: false
    };
  },
  computed: {
    ...mapGetters({
        ...
        exportDataState: 'storkReportExport_4_state'
    })
  },
  watch: {
    exportDataState(data){
      ...
      if(data.success){
        ...
        var title = "盘点统计报表" + this.getNowDateTime();
  ...
  methods: {
    exportData(){
      this.$store.dispatch('inventoryReportExport', this.ruleForm).then(()=>{
