<template>
  <div class="home">
    ...
    <div :style="{'height':height+'px', 'padding-left':'4px', 'overflow-y': 'scroll','background-color': '#F4F5FA' }">
      <el-row style="margin-top: 8px" >
              ...
              <div class="Management_sock_con">
                <div class="triggerAreport"></div>
                  <el-row>
                    <el-col :span="6">
                      <p>收支结余</p>
                      <p class="monthReport1" @click="routeLink" >{{headerDateGainInfo.MONEY1 ? headerDateGainInfo.MONEY1 : '--'}}</p>
                      <p>昨日：<i class="Report1">{{headerDateGainInfo.MONEY2 ? headerDateGainInfo.MONEY2 : '--'}}</i></p>
                    </el-col>
                    <el-col :span="6">
                      <p>消费金额</p>
                      <p class="monthReport2">{{headerDateSaleInfo.SUMMONEY1 ? headerDateSaleInfo.SUMMONEY1 : '--'}}</p>
                      <p>昨日：<i class="Report2">{{headerDateSaleInfo.SUMMONEY2 ? headerDateSaleInfo.SUMMONEY2 : '--'}}</i></p>
                    </el-col>
                    <el-col :span="6">
                      <p>充值金额</p>
                      <p class="monthReport4">{{headerDateAddInfo.SUMMONEY1 ? headerDateAddInfo.SUMMONEY1 : '--'}}</p>
                      <p>昨日：<i class="Report4">{{headerDateAddInfo.SUMMONEY2 ? headerDateAddInfo.SUMMONEY2 : '--'}}</i></p>
                    </el-col>
                    <el-col :span="6">
                      <p>新增会员</p>
                      <p class="systemRemind2">{{homedaycombackobj.ToDayRegVipCount ? homedaycombackobj.ToDayRegVipCount :'--'}}</p>
                      <p>昨日：<i class="systemRemind3">{{homedaycombackobj.ToDayRegVipCount2 ? homedaycombackobj.ToDayRegVipCount2 : '--'}}</i></p>
                    </el-col>
                  </el-row>
              </div>
            </div>
        ...
        <el-col :span="5" class="pull-right">
          <div class="bgwhitecolor home-border-r" style="padding:0; height:170px;">
              <img src='static/images/banner1.png' style="width:100%;height:170px; border-radius:2px ">
          </div>
        </el-col>
      </el-row>
      ...
      <el-row style="margin-top: 8px">
                ...
                <div class="Management_sock fontsizeclass" style="padding-bottom: 0px">
                  <el-dropdown trigger="click" @command="handleCommand">
                    <span class="el-dropdown-link">
                      {{curDateType == 0 ? '近七天' : '本月'}}销售情况<i class="el-icon-arrow-down el-icon--right"></i>
                    </span>
                    <el-dropdown-menu slot="dropdown">
                      <el-dropdown-item command="0">七天</el-dropdown-item>
                      <el-dropdown-item command="1">本月</el-dropdown-item>
                      <el-dropdown-item command="2" style="color:#137deb">查看详细分析</el-dropdown-item>
                    </el-dropdown-menu>
                  </el-dropdown>
                </div>
                <div id="main" style="height:280px;position: relative" :loading="mainloading" element-loading-text='图表数据加载中...' v-loading="mainloading"></div>
              </div>
            </el-col>
          </el-row>
                ...
                <div class="Management_sock">
                  ...
                  <div class="Management_sock_con">
                    ...
                    <el-row>
                      <el-col :span="6">
                        <p>剩余库存</p>
                        <p class="monthReport1" @click="stockChange()">{{stockQty != '' ? stockQty : '--'}}</p>
                      </el-col>
                      <el-col :span="6">
                        <p>本月生日</p>
                        <p class="monthReport2" @click="brithdayChange()">{{noticeInfo.BirthVip != '' ? noticeInfo.BirthVip : '--'}}</p>
                      </el-col>
                      <el-col :span="6">
                        <p>会员过期提醒</p>
                        <p class="monthReport4" @click="brithdayChange()">{{noticeInfo.InvallVip ? noticeInfo.InvallVip : '--'}}</p>
                      </el-col>
                      <el-col :span="6">
                        <p>本月新增会员</p>
                        <p class="monthReport4" @click="addMember()">{{noticeInfo.AddVip ? noticeInfo.AddVip : '--'}}</p>
                      </el-col>
                    </el-row>
                  </div>
                </div>
              ...
              <div class="bgwhitecolor home-border-r">
                <el-row :gutter="10">
                  <el-col :span="24" class="p-top-md">
                    ...
                    <el-select size='mini' v-model="curSortId" @change='changePaiHang(curSortId)' style="width: 72px; ">
                      <el-option v-for='(item,index) in listPaiHang' :key="index" :label="item.name" :value="item.id"></el-option>
                    </el-select>
                  </el-col>
                </el-row>
        ...
        <el-col :span="5">
          <el-row class="rightSmallTitle" v-for='(item, index) in rigthList' :key="index" style="margin-bottom:8px" >
            <div v-if="index == 0 || index == 1" class="bgwhitecolor home-border-r toolItem" style="height: 90px; padding:12px 12px">
              <span>
            ...
            <div v-if="index == 2" class="bgwhitecolor home-border-r toolItem" style="height: 90px; padding:12px 12px">
              <el-popover
                placement="left"
                width="250"
                trigger="click">
            ...
            <div v-if="index == 3" class="bgwhitecolor home-border-r toolItem" style="height: 90px; padding:12px 12px">
              <el-popover
                placement="left"
                width="300"
                title="意见反馈"
                ...
                <el-form :model="ruleForm2" status-icon :rules="rules2" ref="ruleForm2" label-width="0">
                  ...
                  <el-form-item label prop="Remark">
                    <el-input type="textarea" v-model="ruleForm2.Remark" placeholder="请留下您的宝贵意见"></el-input>
                  </el-form-item>
            ...
            <div v-if="index == 4" class="bgwhitecolor home-border-r toolItem" style="height: 90px; padding:12px 12px">
              <el-popover
                placement="left"
                width="250"
                trigger="click">
...
export default {
  data() {
    ...
    return {
      ...
      stockQty:'',
      listPaiHang:[{id: 0, name: '今天'}, {id: 1, name: '昨天'} , {id: 2, name: '本周'}, {id: 3, name: '本月'}],
      tableData1 : [],
      tableData2 : [],
      tableData3 : [],
      headerDateGainInfo:{
  ...
  methods: {
    ...
    getNewData(BeginDate, EndDate){
      this.$store.dispatch('GetPaiHangGroup', { BeginDate : BeginDate, EndDate: EndDate}).then(()=>{
        this.loading3 = true
      })
    },
    handleCommand(val){  // 销售情况时间选择
...
.groupStyle .group_cont div{
  width: 100%; float:left; height: 24px; color:#868686
}
.emptyData{
  width: 100%;
  float: left;
