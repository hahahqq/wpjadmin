...
// actions
const actions = {
    setMallDisplaySaleQty({ commit }, data) { // 商品销量设置
    ...
    setMallDisplayEvaluate({ commit }, data) { // 商品评价设置
        let userInfo = getUserInfo();
        let sendData = {
    ...
    setMallMinMoney({ commit }, data) { // 最低消费设置
        let userInfo = getUserInfo();
        let sendData = {
    ...
    setMallAutoOrder({ commit }, data) { // 自动关闭订单设置
        let userInfo = getUserInfo();
        let sendData = {
    ...
    setMallAutoSaleDay({ commit }, data) { // 自动收货时间设置
        let userInfo = getUserInfo();
        let sendData = {
    ...
    setMallReduceStockType({ commit }, data) { // 减库存设置
        let userInfo = getUserInfo();
        let sendData = {
