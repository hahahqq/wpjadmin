  ...
  <div>
    <section>
      <el-input
        type="default"
        size="small"
        v-model="searchGoodsVal"
        placeholder="输入商品名称/货号/条码"
        @keyup.enter.native="getGoodsData(searchGoodsVal)"
        class="pull-right-"
        style="width: 300px"
      >
        <el-button slot="append" type="default" size="small" @click="getGoodsData(searchGoodsVal)">
          查询
        </el-button>
      </el-input>
    </section>
      ...
      <el-table-column label="图片" align="center" width="80">
        <template slot-scope="scope">
          <el-tooltip placement="right-start">
            <div slot="content">
              <img
                :src="scope.row.IMAGE != undefined ? scope.row.IMAGE : defaultImg"
    ...
    <el-dialog :title="titleName" :visible.sync="isAddGoodsGroup" width="70%" append-to-body>
      ...
      <el-collapse :accordion="true" class="collapseStyle">
        <el-collapse-item>
          <template slot="title">
            <span style="font-size: 16px; color: red">显示剩余库存</span>
          </template>
