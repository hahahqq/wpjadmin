  ...
  <div class="Recharge">
    <el-container>
      <el-container style="height:400px;">
        <el-aside width="50%" style="position:relative; background:none; text-align:left; line-height:normal">
          <div class="Rechargeleft">
            <ul>
              ...
              <li v-if="rechargestatus == 4 ? false : true">
                <span>优惠</span>
                  <el-input v-model="discountMoney" class="pull-right com_fontsize" @keyup.native="handleInput2" placeholder="0.00">
                </el-input>
              </li>
              <li>
                <span class="com_color">{{payName}}</span>
                <el-input v-model="payprice" class="pull-right paytypeMoney com_fontsize" :disabled="disabled" @keyup.native="handleInput3" style="width: 187px;">
                </el-input>
              </li>
              <li style="border-bottom:none; line-height:30px">
                <span class="pull-left" style="color:#ff5722" v-if="couponcode!=''"> <i style="width: 60px; float:left">优惠券</i> <i class="discountpon"> ￥{{couponcodemoney}}</i><i class="el-icon-circle-close p-left-xs" @click="delcoupon"></i></span>
                <span class="pull-right" v-if='!disabled'>找零<i class="Givechange">{{pocketmoney}}</i></span>
              </li>
              <li style="border-bottom:none; line-height:30px">
                <span class="pull-left" style="color:#ff5722" v-if="integralToGoods != 0"> <i style="width: 60px; float:left">积分抵现</i> <i class="discountpon"> ￥{{integralToGoods}}</i> <i class="el-icon-circle-close p-left-xs" @click="delIntegral"></i></span>
              </li>
            </ul>
          </div>
            ...
            <div class="showpayList overflowscroll" style="min-height: 200px; max-height:235px;" v-else>
              <el-row>
                <el-col :span="12" v-for="(item, index) in rechargeListList" :key="index"  style="margin-top: 10px">
                  ...
                  <el-input v-model="item.money" v-on:focus='focusCur(item, index)' v-on:input="clickCurInput(item, index)" type="number" size="small" style="width: 100px" placeholder="0.00"></el-input>
                </el-col>
              </el-row>
            </div>
          ...
          <div class="footersock pull-left full-width">
            <div class="footerleft pull-left">
              <div class="footerleftspan pull-left">
                <span>实收</span>
                <strong style="font-size:20pt; color:#f9484C; font-family:'宋体'" id="netreceiptsmoney">
                  <span style='font-size:10pt'> ￥</span>{{receivableprice}} 
                  <el-button type="text" @click="modifyMoney" style="font-size:18px; color:#999" icon="el-icon-edit-outline"></el-button>
                </strong>
              </div>
              <div class="footerleftspan pull-left" v-if="vipID != ''">
                <span>获得积分 :</span>
                <strong id="Obtainintegral">{{gerintegral}}</strong>
                <el-select size="small" style="width:150px" v-model="value" placeholder="请选择积分" @change="selectGetintegral">
                  <el-option v-for="item in integraltList" :key="item.ID" :label="item.NAME" :value="item.RATE">
                  </el-option>
                </el-select>
              </div>
            </div>
            <div class="footerright pull-right">
              <el-button type="info" @click="closeModal"> 取消
                <!-- <template>
                  <div style="font-size:14px; font-weight: bold">取消</div>
                  <div style="margin-top: 6px"> ( ESC )</div>
                </template> -->
              </el-button>
              <el-button type="primary" :disabled="loading == true" @click="CashPayok"> 确认支付
                <!-- <template>
  ...
  methods: {
    submitUseIntegral(){
      ...
      if (this.formData.deductMoney != '') {
        ...
        this.gerintegral = this.value == 0 ? 0 : Math.ceil(this.CompanyConfig.INTEGRALTYPERATE * totlePrice);
    ...
    modifyMoney(){
      this.$prompt('', '修改实收金额', {
          ...
          this.gerintegral = value == 0 ? 0 : Math.ceil(this.CompanyConfig.INTEGRALTYPERATE * value);
          nscreenexCodeFun(4, String(this.receivableprice));
    ...
    inpIntegral(){
      ...
      if(canUseIntegral > this.vipItem.INTEGRAL){
        ...
        this.$message.warning('最多使用 '+ this.vipItem.INTEGRAL +' 积分抵现 '+ canUseIntegral / this.CompanyConfig.INTEGRALRATE +' 元 ！')
    ...
    focusCur(item, index){
      let param = this.rechargeListList, newParam1 = [], newParam2 = []
      // console.log(param)
    ...
    clickCurInput(item, index){
      ...
      let param = this.rechargeListList, newParam = [], newParam1 = [], newParam2 = []
      for(var _i in param){
        if(param[_i].money != ""){
      ...
      if(newParam1.length == 2){
        ...
        // let haveMoney = param.findIndex(item => item.money != '' )
        // console.log(haveMoney)
        // for(var k in param){
        //   if(param[k].money != ''){
        //     param[k].money = this.totalprice
        //   }
        // }
        // console.log(param)
    ...
    CashPayok() {
      console.log(this.rechargeListList)
      return
