  ...
  <div>
    <el-row>
      <el-col :span="16" class="member-header">
        <div class="center-title p-left-sm" style="border-right: 0" v-if="$route.name == '首页'">
          {{ shopName }}
        </div>
        ...
        <div class="center-cont" v-else-if="$route.name != '首页'">
          <span v-if="$route.name == '条码打印'">
            <a style="cursor: pointer" @click="$router.push({ path: '/good/GoodsBarCode' })">
              单品条码
            </a>
            / {{ $route.name }}
          </span>
