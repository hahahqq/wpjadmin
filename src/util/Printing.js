...
const GoodsConsumption = function(dayinjson, ItemsHeaderType) {
  ...
  pageWidth = printType == 1 ? 800 : 570  //小票宽度
  ...
  if (!pringInfo || pringInfo.ddList[0].isShow) { // 标题
    COMPANYNAME = pringInfo.ddList[0].value
    shopTypename = pringInfo.ddList[0].value
  ...
  if(ItemsHeaderType == 14 || ItemsHeaderType == 15){
    ...
    if (!pringInfo || pringInfo.ddList1[3].isShow) { // 收银员
      hPos = PrintLODOPcommon('销售员:', dealFilterDate(Objvalue.SALEEMPNAME), hPos);
    }
  }

  hPos = PrintLODOPcommonbold(hPos, pageWidth, xianHeight);

  let gw1 = pageLeft + pageLabel - 20, gw2 = 40;
  if (ItemsHeaderType == 1 || ItemsHeaderType == 3) {
    if (ItemsHeaderType == 1) {
      ...
      PrintLODOP_Text(hPos, gw1 - 5, pageWidth, rowHeight, "折扣", { });
      PrintLODOP_Text(hPos, gw1 + gw2 * 1 - 10, gw2, rowHeight, "单价", { Alignment: 2 });
      PrintLODOP_Text(hPos, gw1 + gw2 * 2 - 10, gw2, rowHeight, "数量", { Alignment: 2});
      PrintLODOP_Text(hPos, gw1 + gw2 * 3 - 10, pageWidth, rowHeight, "小计", {  });
      hPos += rowHeight;
      ...
      for (let i = 0; i < dayinjson.GoodsObj.length; i++) {
        ...
        let height = String(goodNameCodeColorSize).length / (pageWidth == 800 ? 24 : 19)
        ...
        PrintLODOP_Text(hPos, 1, gw1 , rowHeight, goodsPrice, { TextNeatRow: true, LineSpacing: 0.3 });
        PrintLODOP_Text(hPos, gw1 - 5, pageWidth, rowHeight, DISCOUNT, {  });
        PrintLODOP_Text(hPos, gw1 + gw2 * 1 - 10, gw2, rowHeight, PRICE, { Alignment: 2 });
        PrintLODOP_Text(hPos, gw1 + gw2 * 2 - 10, gw2, rowHeight, qty, { Alignment: 2 });
    ...
    hPos = PrintLODOPcommon('实收金额:', PAYMONEY, hPos);
    ...
    PrintLODOP_Text(hPos, gw1, gw2, rowHeight, GOODSQTY, { Alignment: 2 });
    PrintLODOP_Text(hPos, gw1 + gw2, gw2, rowHeight, showPurPrice == true ? PAYMONEY : '****', { Alignment: 2});
  ...
  if(ItemsHeaderType == 14){
    ...
    hPos = PrintLODOPcommon('欠供应商款:', dealFilterDate(Objvalue.SUPPLIERCURMONEY), hPos)
  ...
  if(ItemsHeaderType != 14 && ItemsHeaderType != 15 && ItemsHeaderType != 16){
    ...
    if (!pringInfo || pringInfo.ddList[1].isShow) { // 注脚
      ...
      let title = pringInfo.ddList[1].value != '' ? pringInfo.ddList[1].value : "谢谢惠顾，欢迎下次光临!"
