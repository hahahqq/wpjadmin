...
<el-container>
  ...
  <el-container>
    ...
    <el-container :style="`height:${windowHeight}px; overflow:auto`">
        <member v-if="current==0"></member>
        <balance v-if="current==1"></balance>
        <registered v-if="current==2"></registered>
        <adjustment v-if="current==3"></adjustment>
    </el-container>
  </el-container>
</el-container>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            current: 0,
            businessList:[{id:'001',name:"会员分析",number: "91040407"},{id:'002',name:"余额查询",number: "91040413"},{id:'003',name:"注册统计",number:"91040414"},{id:'004',name:"调整统计",number:"91040415"}],
    ...
    created() {
        //判断是否有权限
        // let agentPermission = getUserInfo().List
        // let arr = agentPermission.filter(element => element.MODULECODE == "210040401");
        // if (arr.length > 0 && !this.isPurViewFun(arr[0].MODULECODE)) {
        //     this.current = 2;
        // }
    ...
    components: {
        member: () => import("@/views/reports/analysis/member"),
        balance: () => import("@/views/reports/member/balance"),
        registered: () => import("@/views/reports/member/registered"),
        adjustment: () => import("@/views/reports/member/adjustment"),
    }
};
</script>
<style scoped>
