  ...
  <div class="report row-flex flex-start m-top-sm">
    <section class="bg-white marginLR-sm paddingTB-sm paddingLR-md full-width" v-loading="loading">
        <el-form label-width="66px">
            ...
            <el-form-item label='销售员' class="half">
                <el-select v-model="ruleForm.SaleEmpId" size='small' clearable placeholder="请选择销售员" class="full-width" >
                    <el-option v-for="(item,i) in empdataList" :key="i" :label="item.NAME" :value="item.ID"></el-option>
                </el-select>
            </el-form-item>
            ...
            <el-form-item label='分类' class="half">
                <el-select v-model="ruleForm.TypeId" clearable size='small' placeholder="请选择分类" class="full-width">
      ...
      <div class='full-width pull-left m-bottom-sm reportInfo'>
        共销售 <span> {{dataObj.NUM ? dataObj.NUM : 0}} </span> 笔 , 
        销售数量 <span> {{dataObj.QTY ? dataObj.QTY : 0}} </span> , 
        销售金额 <span> {{dataObj.MONEY ? dataObj.MONEY : 0}} </span>
      </div>
      ...
      >
        ...
        <el-table-column align="center" prop='RECEVINGPIRCE' label="采购价">
          <template slot-scope="scope">
            {{isPurViewFun(91040112) ? scope.row.RECEVINGPIRCE : '****'}}
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" prop='PROFITMONEY' label="毛利">
          <template slot-scope="scope">
            {{isPurViewFun(91040112) ? scope.row.PROFITMONEY : '****'}}
          </template>
        </el-table-column>
...
export default {
  ...
  computed: {
    ...mapGetters({
        ...
        tableList : 'SaleCensusReportList',
        empdataList : "employeeList",
        SaleCensusReportState:'getSaleCensusReportState',
        exportDataState: 'storkReportExport_2_state'
    })
  },
  watch: {
    exportDataState(data){
      ...
      if(data.success){
        ...
        var head = [ "商品名称", "货号", "品牌", "分类", "单位", "数量", "采购价", "零售价", "金额", "毛利"];
        var val = []
        if(this.isPurViewFun(91040112)){
          val =  [ "GOODSNAME", "GOODSCODE", "BRAND", "TYPENAME", "UNITNAME", "QTY", "RECEVINGPIRCE", "PRICE", "MONEY", "PROFITMONEY" ]
        }else{
          val =  [ "GOODSNAME", "GOODSCODE", "BRAND", "TYPENAME", "UNITNAME", "QTY", "***", "PRICE", "MONEY", ""]
        }

        var title = "销售统计报表" + this.getNowDateTime();
        this.export2Excel(head, val, list, title);
      }
    },
    SaleCensusReportState(data){
        this.loading = false
        if(data.success){
          this.pagination =  {
  ...
  methods: {
    exportData(){
      this.$store.dispatch('SaleCensusReportExport', this.ruleForm).then(()=>{
        this.exportLoading = true
      })
    },
    searchData(){
      this.$store.dispatch('GetSaleCensusReport', this.ruleForm).then(()=>{
        this.loading = true
      })
    },
