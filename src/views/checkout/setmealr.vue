  ...
  <div class="s-botton-setmealr" style="height: 100%">
          ...
          <el-form label-width="80px" class="fastc_top">
            <div class="selecttaocan">
              ...
              <ul class="showtaocanList" v-loading="loading" v-show="taotandatalist.length > 0">
                <li
                  v-for="(item, index) in taotandatalist"
                  :key="index"
                  @click="selectoneTotle(item)"
                >
    ...
    <el-dialog title="选择业绩员工" :visible.sync="dialogFormVisible">
      <div class="employeetitle">
        充值金额：
        <i class="com_color">{{ setmealrPrice }}</i>
        元，按金额的
        <i class="com_color">{{ ruleFormsock.getfastcExtract }}</i>
        提成，员工可的
        <i class="com_color">{{ ruleFormsock.ExtractMoney }}</i>
        元提成
      </div>
