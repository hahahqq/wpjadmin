<template>
  <div class="t-botton-timescountc" ref="elememt">
    <headerPage
      @returnOrderList1="returnOrderList1"
      :updataDate="date"
      @returnOrderObj1="returnOrderObj1"
    ></headerPage>
    <!-- 列表 -->
    <div ref="addsockheight" class="overflowscroll">
      <div class="tableStyle tableLineHeight">
        ...
        >
          ...
          <el-table-column align="center" label="实销价">
            <template slot-scope="scope">
              ...
              <span v-show="scope.row.notEdit">
                <el-input-number
                  size="mini"
                  :style="`width:${scope.row.Remark == '' ? '120' : '110'}px`"
                  controls-position="right"
                  :min="0"
              ...
              <span
                v-if="scope.row.Remark != '' && scope.row.Remark != undefined"
          ...
          <!-- <div slot="empty" class="emptyBg m-top-md">
    ...
    <div class="c_footer" ref="footer">
      <el-row :gutter="24">
        <el-col
          :span="8"
          style="padding: 16px 16px; position: relative; border-right: 2px solid #e4e4e4"
        >
          <el-select
            v-model="searchText"
            size="small"
          ...
          <el-row style="height: 60px">
            <el-col :span="7">
              <el-button
                ...
                plain
                @click="saveGuaDan"
                :disabled="shopDataList.length == 0"
              >
                挂 单 - F8
              </el-button>
            </el-col>
            <el-col :span="7">
              <el-badge type="danger" :value="guadancxlistBillCount">
                <el-button
                  type="primary"
                  :disabled="guadancxlistBillCount == 0"
                  plain
                  @click="isShowGuaDan"
                >
                  取 单 - F9
    ...
    >
      <guadanPage
        @closeModal="
          ...
          $refs.input.focus();
        "
        v-on:pullgoodsList="pullgoodsList"
        style="max-width: 100%; height: 400px; overflow: hidden"
      ></guadanPage>
    </el-dialog>
...
export default {
  ...
  watch: {
    ...
    rechargeListState(data) {
      //支付方式
      let param = data.data.List,
        newParam = [];
      if (this.isVip == true) {
        newParam = param;
      } else {
    ...
    savePayMoneyState(data) {
      ...
      if (data.success) {
        ...
        //   this.$store.dispatch("getGoodsListIncludeSingle", this.pageData1).then(()=>{
    ...
    goodsListIncludeSingleState(data) {
      let _obj = JSON.stringify(data.data.obj),
        nullObj = true,
      ...
      if (data.success) {
        ...
        if (nullObj == false && nullList == false) {
          // 单品检索
          let info = data.data.obj,
            Price,
            Remark,
            Discount;
          if (info.DISPRICE != undefined) {
            if (info.ISVIPDISCOUNT == true) {
              Price = parseFloat(info.DISPRICE * this.vipnews.DISCOUNT * 100) / 100;
              if (this.vipnews.DISCOUNT != 1) {
                Remark =
                  "（限时特价" + info.DISPRICE + "元, 会员折扣" + this.vipnews.DISCOUNT + ")";
              } else {
                Remark = "（限时特价" + info.DISPRICE + "元)";
              }
              Discount = (Price / info.PRICE).toFixed(2);
            } else {
              Price = info.DISPRICE;
              Remark = "（限时特价" + info.DISPRICE + "元)";
              Discount = (info.DISPRICE / info.PRICE).toFixed(2);
            }
            if (info.VIPPRICE != undefined && info.VIPPRICE != 0 && this.VipId != "") {
              if (info.VIPPRICE < Price) {
                Price = info.VIPPRICE;
                Discount = 1;
                Remark = "（会员价" + info.VIPPRICE + "元)";
              }
            }
          } else {
            console.log(this.VipId);
            if (info.VIPPRICE != undefined && info.VIPPRICE != 0 && this.VipId != "") {
              let disPrice = info.PRICE * this.vipnews.DISCOUNT;
              if (disPrice < info.VIPPRICE) {
                Price = disPrice;
              ...
              if (this.vipnews.DISCOUNT != 1 && this.VipId != "") {
                Price = info.PRICE * this.vipnews.DISCOUNT;
          ...
          let info1 = {
            ...
            Remark: Remark,
            notEdit: false,
            DISPRICE: info.DISPRICE,
            ISVIPDISCOUNT: info.ISVIPDISCOUNT,
            Discount: Discount,
        ...
        } else if (nullList == false && nullObj) {
          // 非单品检索，弹出商品查询弹窗
          console.log("有List 无obj");
          this.isShowList1 = true;
          this.goodsVal = this.searchVal;
          this.goodsSearchList = data.data.List;
        ...
        if (info.length == 0) {
          ...
          if (info[0].DISPRICE != undefined) {
            if (info[0].ISVIPDISCOUNT == true) {
              ...
              if (this.vipnews.DISCOUNT != 1) {
                Remark =
                  "（限时特价" + info[0].DISPRICE + "元, 会员折扣" + this.vipnews.DISCOUNT + ")";
              } else {
  ...
  methods: {
    ...
    btnPayMoney() {
      ...
      if (this.shopDataList.length != 0) {
        ...
        this.$store.state.commodityc.payMoneyVipID =
          this.VipId == "" || this.VipId == undefined ? "" : this.VipId;
    ...
    returnOrderList1(data) {
      ...
      for (let i = 0; i < param.length; i++) {
        newArr.push({
          ...
          totalMoney:
            (
              param[i].BALANCEPRICE *
              (param[i].BALANCEPRICE / param[i].GOODSPRICE) *
              param[i].QTY
            ).toFixed(2) * -1,
        ...
        for (let i in data) {
          ...
          if (data[i].DISPRICE != undefined) {
            if (data[i].ISVIPDISCOUNT == true) {
              ...
              if (this.vipnews.DISCOUNT != 1) {
                Remark =
                  "（限时特价" + data[i].DISPRICE + "元, 会员折扣" + this.vipnews.DISCOUNT + ")";
              } else {
                Remark = "（限时特价" + data[i].DISPRICE + "元)";
              }
      ...
      for (let i = 0; i < param.length; i++) {
        ...
        if (param[i].DISPRICE != undefined) {
          if (param[i].ISVIPDISCOUNT == true) {
            ...
            if (this.vipnews.DISCOUNT != 1) {
              Remark =
                "（限时特价" + param[i].DISPRICE + "元, 会员折扣" + this.vipnews.DISCOUNT + ")";
            } else {
  ...
  components: {
    ...
    shopQueryChose: () => import("@/components/checkout/shopQueryChose"),
...
<style scoped>
.tableLineHeight >>> .el-table--border::after,
