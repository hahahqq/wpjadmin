<template>
  <div class="t-botton-timescountc" ref="elememt">
    ...
    <el-dialog title="商品查询" :visible.sync="isShowList1" width="70%" :close-on-click-modal='false' style="max-width:100%;" append-to-body>
       <shopQueryChose @closeModal="isShowList1=false; searchVal=''; $refs.input.focus()" :goodsValSearch='goodsVal' :goodsSearchList='goodsSearchList' @getSelectList="getSelectList"></shopQueryChose>
    </el-dialog>
...
export default {
  ...
  watch: {
   ...
   goodsListIncludeSingleState(data){
      ...
      let _obj = JSON.stringify(data.data.obj), nullObj = true, _list = data.data.List, nullList = true
      console.log(_obj)
      if(_obj == 'null' || _obj == '{}'){
         nullObj = true
      }else{
      ...
      if(data.success){
         ...
         if(nullObj == false && nullList == false){  // 单品检索
            let info = data.data.obj, Price, Remark, Discount;
            if(info.DISPRICE != undefined){
               if(info.ISVIPDISCOUNT == true){
                  ...
                  if(this.vipnews.DISCOUNT != 1){
                     Remark = '（限时特价' + info.DISPRICE + '元, 会员折扣' + this.vipnews.DISCOUNT +')'
                  }else{
         ...
         }else if(nullList == false && nullObj ){   // 非单品检索，弹出商品查询弹窗
...
<style scoped>
.tableLineHeight>>>.el-table--border::after, .el-table--group::after, .el-table::before{
