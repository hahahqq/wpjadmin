                    ...
                    <div style="max-width:900px;">
                        ...
                        <div style="max-width:490px;" class="m-rop-sm">
                            <div class="padding-sm rounded-xs" style="background:#edf5f9">
                                ...
                                <span class='pull-left'>剩余短信 <i style="color:#f00">{{companyInfo.SMSNumber}}</i> 条</span>
                                <span>
                                    <el-button @click="onSubmit" type="primary" size="small" :loading="loading">发 送</el-button>
                                </span>
                                </div>
                            </div>
                            <div v-show="!isAll">
                            ...
                            <div class="font-12">
                                <p class="m-top-sm">已输入 <span style="color:#f00"> {{SmsRemark.length + fixFontLength}} </span> 个字（含退订回T、会员名和【短信签名】），当前内容按 <span style="color:#f00"> {{onStatistics}} </span>条计费  </p>
                                <p class='m-top-sm' style='color:#999'>注: 因会员名长度不一,计算时会有误差,以实际收到内容为准</p>
                                <div class="marginTB-md" >
                                    <div class="m-bottom-sm font-14">
                                        <b>短信须知：</b>
                                    </div>
                                    <ul>
                                        <li class="marginTB-xs">1、按70字作为一条短信计算，若超过则之后更多内容按67字作为一条计算（含退订回T、会员名和【短信签名】）；</li>

                                        <li class="marginTB-xs">2、少量短信30分钟左右到达，群发短信60分钟左右到达，节假日略有延迟；</li>

                                        <li class="marginTB-xs">3、短信发送时间：8:00-18:00;</li>

                                        <li
                                            class="marginTB-xs"
                                        >4、根据工信部《通信短信息服务管理规定》要求，所有营销短息需在结尾加退订回T，旺铺记已在短信内预置，无需重复添加；</li>
...
export default {
    ...
    methods: {
        ...
        onSubmit() {
            ...
            this.$store.dispatch("setSMSAction", {
