<template>
    <section class="mallOrder-item bg-white p-bottom-sm" v-loading="loading">
        ...
        <div class="padding-md">
            <div
                class="dealHistory paddingLR-lg relative overflow-hidden"
                :style="{height:(seeHistory?'auto':'95px')}"
            >
                <el-timeline :reverse="true">
                    <el-timeline-item
                        ...
                        placement="top"
                    >
                        <div style="min-height:50px;">{{activity.content}}</div>
                    </el-timeline-item>
                </el-timeline>
            </div>
            <div class="paddingLR-lg paddingTB-sm">
                <el-button type="text" @click="seeHistory=!seeHistory">
                    <span v-text="seeHistory?'收取':'查看更多'"></span>
                </el-button>
            </div>
        </div> -->
        <div class="padding-xs bg-elMain"></div>
...
export default {
    ...
    methods: {
        ...
        defaultData() {
            ...
            this.dealHistory = [
                {
                    content: this.filterTime(new Date(this.dataInfo.BILLDATE)),
                    title: "买家申请退款，申请时间"
                }
            ];
            if (this.activeIdx == 1) {
            ...
            if (this.activeIdx > 1) {
                let dstr = this.dataInfo.AGREETIME
                    ? this.filterTime(new Date(this.dataInfo.AGREETIME))
                    : "";
                this.dealHistory.push({
                    content: dstr,
                    title: "同意退款，处理时间"
