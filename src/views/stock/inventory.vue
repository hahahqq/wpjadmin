      ...
      <el-container>
          ...
          <el-container>
            <div class="content-new-fex"> 
              ...
              <historyPage v-on:isAddNewBill_I='isAddNewBill_I' v-if="current==1"></historyPage>
            </div>
          </el-container>
      </el-container>
...
import addNewPage from "./inventory/addNew.vue";
import historyPage from "./inventory/history.vue";
...
export default {
  ...
  data() {
    return {
      current: 0,
      businessList:[{id:'001',name:"库存盘点"},{id:'002',name:"库存盘点历史"}],
  ...
  methods:{
    ...
    clearSetItem(){
      this.$store.dispatch('cgdInventoryDataList', '').then(()=>{
        sessionStorage.setItem('theGoodsObj_I', JSON.stringify({}))
        sessionStorage.setItem('countingUserList_I', JSON.stringify([]))
      })
    }
  },
  destroyed: function () {
