  ...
  <div class="inventory">
    ...
    <div class="content-table4">
      <div class="content-table-center">
        <!-- <el-tabs v-model="editableTabsValue" type="card" closable @tab-remove="removeTab" style="width: 88%; float:left">
          <el-tab-pane
        ...
        <!-- <el-button size="small" style="float:right" type="primary" plain icon="el-icon-plus" @click="addTab(editableTabsValue)">新增子单</el-button> -->
          ...
          >
            ...
            <el-table-column label="数量" align="center" width="150">
              <template slot-scope='scope'>
                ...
                <el-input-number v-if='scope.row.isEdit && scope.row.SIZENAME == ""' size='small' style='width:110px' disabled controls-position="right" :min="0" v-model.trim="scope.row.QTY" @input="totalfun" placeholder="请输入"></el-input-number>
                <el-input-number v-if='scope.row.isEdit && scope.row.SIZENAME != ""' size='small' style='width:110px' controls-position="right" :min="0" v-model.trim="scope.row.QTY" @input="totalfun" placeholder="请输入"></el-input-number>
              </template>
            </el-table-column>
        ...
        <div class='marginTB-sm'>
          ...
          <span class='text-right' style='float:right'>
            ...
            <el-button type="primary" @click="handleSubmit(1)" :disabled="theGoodsList.length == 0" :loading="loading">盘点</el-button>
          </span>
        </div>
      </div>
    </div>
...
export default {
  ...
  watch: {
    ...
    goodsState(data) {
      if (data.success) {
        let storedCount = []
        let result = data.data.StockList
        ...
        this.stockList = data.data.StockList   // 商品库存列表
  ...
  mounted() {
    ...
    if(theGoodsList.length != 0){
        let newArr = []
        for(var i in theGoodsList){
          theGoodsList[i].QTY = theGoodsList[i].STOCKQTY
          newArr.push(theGoodsList[i])
        }
        this.theGoodsList = newArr.concat({
            GOODSNAME: "",
            GOODSID:'',
